  ...
  <div>
    <div class="top-tool">
      ...
      <div class="search-tool">
        <el-input v-model="searchInfo.desensitizationName"
                  class="search-input"
                  size="mini"
                  :clearable="true"
                  placeholder="请输入规则名称">
        </el-input>
        <el-input v-model="searchInfo.desensitizationValue"
                  class="search-input"
                  size="mini"
                  :clearable="true"
                  placeholder="请输入规则内容">
...
<script>
import { deleteRule, getRulePage } from '@/api/dataCenter/dataGovernance'
  ...
  methods: {
    ...
    deleteItem(rowId) {
      ...
      this.$confirm("确认要删除该信息吗？", {
        ...
        .then(() => {
          deleteRule({ ids:id }).then((reponse) => {
            if (reponse.code == 200) {
              this.$message({
                message: reponse.msg,
                type: "success",
