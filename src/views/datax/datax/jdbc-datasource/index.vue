  ...
  <div class="app-container">
    ...
    >
      <el-form-item label="数据源名称:" prop="datasourceName">
        <el-input v-model="queryParams.datasourceName" placeholder="数据源名称" style="width: 180px;" />
      </el-form-item>
      <el-form-item>
        <el-button
            type="primary"
    ...
    <pagination
      v-show="total>0"
      :total="total"
      :pageNo.sync="queryParams.current"
      :limit.sync="queryParams.size"
      @pagination="fetchData" class="pull-right"
    />

    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" center width="700px">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="88px" class="dialog-form">
        ...
        <el-form-item v-if="jdbc||influxdb" label="用户名">
          <el-input v-model="temp.jdbcUsername" placeholder="用户名"  />
        </el-form-item>
        <el-form-item v-if="visible" v-show="jdbc||influxdb" label="密码">
          <el-input v-model="temp.jdbcPassword" type="password" placeholder="密码" >
            <i slot="suffix" title="显示密码" style="cursor:pointer" class="el-icon-view" @click="changePass('show')" />
          </el-input>
        </el-form-item>
        <el-form-item v-show="jdbc||influxdb" v-else label="密码">
          <el-input v-model="temp.jdbcPassword" type="text" placeholder="密码">
        ...
        <el-form-item v-if="mongodb" label="数据库名称" prop="databaseName">
          <el-input v-model="temp.databaseName" placeholder="数据库名称"  />
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        datasourceName: [{ required: true, message: '请选择数据源名称', trigger: 'blur' }],
        jdbcUsername: [{ required: true, message: '请输入用户名', trigger: 'blur' }],
        jdbcPassword: [{ required: true, message: '请输入密码', trigger: 'blur' }],
        jdbcUrl: [{ required: true, message: '请输入jdbcurl地址', trigger: 'blur' }],
        jdbcDriverClass: [{ required: true, message: '请输入jdbc驱动类', trigger: 'blur' }],
        datasource: [{ required: true, message: '请输入数据源名称', trigger: 'change' }],
        zkAdress: [{ required: true, message: '请输入ZK地址', trigger: 'blur' }],
  ...
  methods: {
    ...
    fetchData() {
      ...
      datasourceApi.list(this.queryParams).then(response => {
