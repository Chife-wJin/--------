  ...
  <div class="app-container">
    ...
    >
      <el-form-item label="任务名称:" prop="jobDesc">
        <el-input v-model="queryParams.jobDesc" placeholder="任务名称" style="width: 180px;" />
      </el-form-item>
      <el-form-item label="所属项目:" prop="projectIds">
        <el-select v-model="projectIds" multiple placeholder="所属项目" style="width: 180px">
          <el-option v-for="item in jobProjectList" :key="item.id" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
      <el-form-item label="任务类型:" prop="glueType">
        <el-select v-model="queryParams.glueType" placeholder="任务类型" style="width: 180px" >
          <el-option v-for="item in glueTypes" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item>
        <el-button v-waves type="primary" size="mini" icon="el-icon-search" @click="handleQuery">
          搜索
        </el-button>
    ...
    <pagination v-show="total>0" :total="total" :pageNo.sync="queryParams.current" :limit.sync="queryParams.size" @pagination="fetchData" class="pull-right"/>



    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" center :before-close="handleClose" width="1200px">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="110px" class="dialog-form">
        <el-row :gutter="20">
          <el-col :span="12">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        jobProject: [{ required: true, message: '请输入所属项目', trigger: 'blur' }],
  ...
  methods: {
    ...
    fetchData() {
      ...
      if (this.projectIds) {
        this.queryParams.projectIds = this.projectIds.toString()
      }

      job.getList(this.queryParams).then(response => {
        const { content } = response
    ...
    // 查看日志
    handlerViewLog(row) {
      this.$router.push({ path: 'jogInfo/index', query: { jobId: row.id }})
    },
    handlerStart(row) {
