...
import FlowExp from '@/views/support/oa/components/flow/Expression'
import { listAllForm } from '@/api/oa/flowable/form'
    ...
    checkValuesEcho(){
      ...
      if (attrs.hasOwnProperty("flowable:assignee")) {
        ...
        if (attrs["flowable:dataType"] === "dynamic") {
          ...
          this.checkValues = that.userList.find(item => item.id == val).name;
          this.selectValues = that.userList.find(item => item.id == val).id;
        }
        // 候选用户
        ...
        if (attrs["flowable:dataType"] === "dynamic") {
          ...
          this.checkValues =  newArr.map(item => item.name).join(',');
    ...
    handleUserSelect(selection) {
      ...
      if (selection) {
        ...
        if (selection instanceof Array) {
          ...
          const name = selection.map(item => item.name);
          that.updateProperties({'flowable:candidateUsers': userIds.join(',')})
          that.checkValues = name.join(',');
        } else {
          that.updateProperties({'flowable:assignee': selection.id})
          that.checkValues = selection.name;
        }
      }
    },
    // 角色选中数据
