<template>
  <div class="app-container">
    ...
    <el-dialog
      ...
      append-to-body
    >
      <div class="test-form form-body">
        <parser
          v-if="Object.keys(formConf).length && formConfOpen"
          :key="new Date().getTime()"
          :form-conf="formConf"
        />
        <form-example
          v-else-if="formUrl && formConfOpen"
          :showOnly="true"
          :btnIsShow="[]"
          :currentFormUrl="formUrl"
          :deployId="formDeployParam.deployId"
        />
      </div>
    </el-dialog>
    ...
    >
      <el-row :gutter="24">
        ...
        <el-col :span="14" :xs="24" class="form-body">
          <parser
            v-if="currentRow"
            :key="new Date().getTime()"
            :form-conf="currentRow"
          />
          <form-example
            v-else-if="currentFormUrl"
            :showOnly="true"
            :btnIsShow="[]"
...
import { getForm, addDeployForm, listForm } from "@/api/oa/flowable/form";
...
import flow from "@/views/support/oa/task/myProcess/send/flow";
...
export default {
    ...
    return {
      ...
      upload: {
        ...
        url:
          ...
          "/center/flowable/definition/import",
      },
      // 查询参数
      queryParams: {
        pageNum: 1,
  ...
  methods: {
    ...
    handleCurrentChange(data) {
      if (data) {
        const { formContent, formUrl } = data;
        if (formContent) {
