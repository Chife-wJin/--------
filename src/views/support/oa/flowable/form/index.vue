    ...
    >
      ...
      <el-table-column label="表单类型" align="center" prop="formType">
        <template slot-scope="scope">
          <span v-if="scope.row.formType == 0">自定义表单</span>
          <span v-else-if="scope.row.formType == 1">业务表单</span>
        </template>
      </el-table-column>
...
import {
  ...
  updateForm,
  exportForm,
} from "@/api/oa/flowable/form";
import Editor from "@/views/support/oa/components/Editor";
import Parser from "@/views/support/oa/components/parser/Parser";
import deathFormAdd from "./deathFormAdd";
export default {
  name: "Form",
  components: {
    Editor,
  ...
  methods: {
    ...
    handleDetail(row) {
      ...
      if (formType == 0) {
        ...
        this.detailRow = { pageType: "查看", row };
        this.detailDialog = true;
        this.$nextTick(() => {
          this.$refs["detailRef"].formInit();
        });
      }
    },
    /** 新增按钮操作(自定义表单) */
    handleAdd() {
      ...
      // this.open = true;
      // this.title = "添加流程表单";
      this.$router.push({ name: "FormConfig", query: { formId: null } });
    },
    // 新增按钮操作(业务表单)
    handleAdd2() {
      this.detailRow = { pageType: "新增" };
      this.detailDialog = true;
      this.$nextTick(() => {
        this.$refs["detailRef"].formInit();
      });
    ...
    handleUpdate(row) {
      ...
      if (formType == 0) {
        ...
        this.detailRow = { pageType: "修改", row };
        this.detailDialog = true;
        this.$nextTick(() => {
          this.$refs["detailRef"].formInit();
        });
