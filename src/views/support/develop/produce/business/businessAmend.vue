<template>
  <div class="amend-content tab-list-content">
        ...
        <el-row :gutter="24">
          <el-col :span="24">
            <el-form-item label="数据源：">
              <el-select v-model="dataOrigin" @change="dataOrignChange">
                <el-option
    ...
    <div class="business-info" v-else>
      <el-form ref="userManageform" :model="form" label-width="90px">
        ...
        >
          ...
          <el-table-column label="java类型" width="160">
            <template slot-scope="{ $index }">
              <el-form-item
                :prop="'columnList.' + $index + '.javaType'"
                label-width="0"
                :rules="rules.javaType"
              >
                <el-select v-model="form.columnList[$index].javaType">
                  <el-option
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        javaType: { required: true, message: "请选择", trigger: "change" },
      },
  ...
  mounted() {
    if (!this.detail.id) {
      ...
      this.getGenTableById(this.detail);
    }
  },
  methods: {
    async getDataOriginList() {
      const response = await dataOriginApi();
      this.dataOriginList = response.data;
    },
    dataOrignChange(val) {
      this.getGenTableAdd({ datasourceId: val });
    },
    async getGenTableAdd(params) {
      const reponse = await genTableAdd(params);
      this.genTableAddData = reponse.data;
      this.genTableName = reponse.data[0].name;
    },
    async onHandleNext() {
      const reponse = await genTableAdd({
        genTableName: this.genTableName,
        datasourceId: this.dataOrigin,
      });
      if (reponse.code === 500) {
        this.businessAdd = true;
      } else {
    ...
    //修改
    async getGenTableById(params) {
      const reponse = await genTableById({
        id: params.id,
    ...
    async getGenTableByName(tableName) {
      const reponse = await genTableByName({
        tableName,
        datasourceId: this.dataOrigin,
      });
      const subject = reponse.data;
    ...
    onSubmit() {
      this.$refs.userManageform.validate(async (valid) => {
        if (valid) {
          ...
          this.form.datasourceId = this.dataOrigin;
          const data = JSON.stringify(this.form);
          const url =
            process.env.VUE_APP_BASE_API + "/center/gen/genTable/save";
          axios({
            url: url,
            method: "post",
            data: data,
